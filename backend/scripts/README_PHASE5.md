# Phase 5F: Comprehensive Test Data

Seed scripts that populate the database with realistic test data covering all
**Phase 5C** (Modules), **Phase 5D** (Workflow), and **Phase 5E** (Scheduling)
features.
...
| File | Purpose |
|------|---------|
| `seed_phase5_data.py` | Creates 700+ records across all Phase 5 collections |
| `clear_phase5_data.py` | Safely removes all Phase 5 test data with confirmation |
...
# 5. (Optional) Clear Phase 5 data afterwards
...
| Role | Email |
|------|-------|
| SuperAdmin | superadmin@phase5.com |
| Admin | alice.morgan@phase5.com |
| Admin | bob.chen@phase5.com |
| Admin | carol.hayes@phase5.com |
| Site Manager | david.park@phase5.com |
| Site Manager | emma.lewis@phase5.com |
| Site Manager | frank.torres@phase5.com |
| Site Manager | grace.kim@phase5.com |
...
- `BaseContract` must have `workflow_state`, `workflow_metadata`, `module_config`,
  `state_changed_at`, `state_changed_by` fields (added in Phase 5D/5F).
