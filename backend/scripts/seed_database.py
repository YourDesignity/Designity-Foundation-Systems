...
from backend.models.contracts import LabourContract, RoleBasedContract, GoodsContract
from backend.models.contracts.base_contract import ContractType
...
async def create_admins() -> List[Admin]:
    ...
    # Create ManagerProfile records for every Site Manager
    # Without this, the /managers/profiles endpoint returns empty
    profile_count = 0
    for admin in admins:
        if admin.role != "Site Manager":
            continue
        try:
            profile_uid = await get_next_uid("manager_profiles")
            profile = ManagerProfile(
                uid=profile_uid,
                admin_id=admin.uid,
                full_name=admin.full_name,
                designation=admin.designation or "Site Supervisor",
                monthly_salary=round(random.uniform(600, 1200), 3),
                allowances=round(random.uniform(50, 200), 3),
                phone=admin.phone or fake.numerify("+965-####-####"),
                address=fake.address(),
                nationality=random.choice(["Kuwaiti", "Indian", "Filipino", "Egyptian", "Pakistani"]),
                date_of_joining=days_ago(random.randint(180, 730)),
                is_active=True,
                created_by_admin_id=1,  # SuperAdmin uid=1
            )
            await profile.insert()
            profile_count += 1
        except Exception as e:
            print(f"   Warning: ManagerProfile for {admin.email} failed: {e}")

    # Create ManagerAttendanceConfig for each manager
    for admin in admins:
        if admin.role != "Site Manager":
            continue
        try:
            cfg_uid = await get_next_uid("manager_attendance_configs")
            cfg = ManagerAttendanceConfig(
                uid=cfg_uid,
                manager_id=admin.uid,
                ...
                configured_by_admin_id=1,  # SuperAdmin
            )
            await cfg.insert()
        except Exception as e:
            pass  # non-critical

    print(f"    Created {profile_count} manager profiles with attendance configs")
    return admins
    ...
    for p_idx in range(NUM_PROJECTS):
        ...
        for _ in range(n_contracts):
            ...
            contract = LabourContract(
                ...
                contract_type=ContractType.DEDICATED_STAFF,
                project_id=proj_uid,
                project_name=proj_name,
                client_name=client,
                start_date=start_dt,
...
async def create_vehicles() -> List[Vehicle]:
    ...
    for i in range(NUM_VEHICLES):
        ...
        vehicle = Vehicle(
            ...
            registration_expiry=(now() + timedelta(days=random.choice([7, 14, 20, 45, 90, 180, 365]))).strftime("%Y-%m-%d"),
            insurance_expiry=(now() + timedelta(days=random.choice([10, 25, 30, 60, 120, 240, 365]))).strftime("%Y-%m-%d"),
        )
...
async def create_designations() -> None:
    """Create company designation master list."""
    print("\n  Creating designations...")
    titles = [d[0] for d in DESIGNATIONS] + ["Supervisor", "Security Guard", "Technician", "Helper", "Site Manager"]
    count = 0
    for title in titles:
        ...
        if not existing:
            ...
            count += 1
    print(f"   OK Created {count} designations")
...
async def create_attendance_records(employees: list, admins: list) -> None:
    """Create 60 days of attendance records for all employees."""
    print("\n  Creating attendance records (60 days)...")
    count = 0
    statuses = ["Present"] * 22 + ["Absent"] * 3 + ["Late"] * 5

    for emp in employees[:30]:  # first 30 employees for performance
        for day_offset in range(60):
            d = days_ago(day_offset)
            if d.weekday() == 4:  # skip Fridays (Kuwait weekend)
                continue
            ...
            try:
                ...
                att = Attendance(
                    ...
                    status=status,
                    check_in=d.replace(hour=8, minute=random.randint(0, 30)).strftime("%H:%M") if status != "Absent" else None,
                    check_out=d.replace(hour=17, minute=random.randint(0, 30)).strftime("%H:%M") if status != "Absent" else None,
                )
                await att.insert()
                count += 1
            except Exception:
...
# ==========================================================
# SECTION: OVERTIME & DEDUCTIONS
# ==========================================================

async def create_overtime_and_deductions(employees: list) -> None:
    """Create realistic overtime and deduction records for last 3 months."""
    print("\n  Creating overtime and deduction records...")
    ot_count = ded_count = 0
    reasons = ["Late arrival", "Absence without notice", "Equipment damage", "Advance recovery"]
    ot_reasons = ["Urgent site work", "Project deadline", "Emergency maintenance", "Client request"]

    for emp in employees[:40]:
        # Overtime: 2-5 records per employee over last 3 months
        for _ in range(random.randint(2, 5)):
            d = days_ago(random.randint(1, 90))
            try:
                uid = await get_next_uid("overtime")
                await Overtime(
                    uid=uid,
                    employee_uid=emp.uid,
                    date=d.strftime("%Y-%m-%d"),
                    hours=random.choice([1.0, 1.5, 2.0, 2.5, 3.0, 4.0]),
                    type=random.choice(["Normal", "Normal", "Normal", "Offday"]),
        ...
        # Deductions: 0-2 records per employee
        for _ in range(random.randint(0, 2)):
            month_offset = random.randint(0, 2)
            d = days_ago(month_offset * 30)
            pay_period = d.strftime("%Y-%m")
            try:
                ...
                await Deduction(
                    ...
                    amount=round(random.uniform(5, 50), 3),
    ...
    print(f"   OK Created {ot_count} overtime records, {ded_count} deduction records")


# ==========================================================
# SECTION: INVOICES (Finance data)
# ==========================================================

async def create_invoices(contracts: list, admins: list) -> None:
    """Create invoice records so the Finance dashboard has real data."""
    print("\n  Creating invoices for finance dashboard...")
    super_admin = next((a for a in admins if a.role == "SuperAdmin"), admins[0])
    active = [c for c in contracts if c.workflow_state in ("ACTIVE", "COMPLETED")]
    count = 0

    for contract in active:
        # 1-4 invoices per active contract
        for i in range(random.randint(1, 4)):
            inv_date = days_ago(random.randint(1, 180))
            amount = round(contract.contract_value * random.uniform(0.15, 0.35), 3)
            status = random.choice(["paid", "paid", "paid", "pending", "overdue"])
            try:
                uid = await get_next_uid("invoices")
                # Invoice uses: invoice_no, project_uid, client_name, date, due_date, items, total_amount, status
                inv = Invoice(
                    uid=uid,
                    invoice_no=f"INV-{uid:04d}",
                    project_uid=contract.project_id,
                    client_name=contract.client_name or "Client",
                    date=inv_date.strftime("%Y-%m-%d"),
                    due_date=(inv_date + timedelta(days=30)).strftime("%Y-%m-%d"),
                    items=[InvoiceItem(
                        description=f"Services — {contract.contract_code}",
                        quantity=1,
                        unit_rate=amount,
                        total=amount,
                    )],
    ...
    print(f"   OK Created {count} invoices")


# ==========================================================
# SECTION: MAINTENANCE LOGS (for real vehicle alerts)
# ==========================================================

async def create_maintenance_logs(vehicles: list) -> None:
    """Create maintenance logs with upcoming due dates so System Alerts work."""
    from backend.models.vehicles import MaintenanceLog
    print("\n  Creating vehicle maintenance logs...")
    count = 0
    service_types = ["Oil Change", "Tyre Rotation", "Full Service", "Brake Check", "Filter Replacement"]

    for v in vehicles:
        # Past service
        service_date = days_ago(random.randint(30, 180))
        # Some vehicles have overdue next service (triggers alerts)
        days_until_next = random.choice([-5, -3, 7, 10, 14, 30, 60, 90, 120])
        next_due = now() + timedelta(days=days_until_next)
        try:
            ...
            log = MaintenanceLog(
                ...
                cost=round(random.uniform(20, 250), 3),
                mileage_at_service=v.current_mileage - random.uniform(1000, 15000),
                notes=f"Routine {random.choice(service_types).lower()} performed.",
            )
            await log.insert()
            count += 1
        except Exception:
    ...
    print(f"   OK Created {count} maintenance logs")
...
async def main() -> None:
    ...
    print(f"\n  This will insert Phase 5 test data into: {DB_NAME}")
    ...
    try:
        if contracts and employees:
            ...
            print("     Skipped module assignments (no contracts or employees)")
    except Exception as e:
    ...
    try:
        if contracts:
            ...
            print("     Skipped workflow history (no contracts)")
    except Exception as e:
    ...
    try:
        if contracts:
            ...
            print("     Skipped approval requests (no contracts)")
    except Exception as e:
    ...
    try:
        if contracts:
            ...
            print("     Skipped scheduled jobs (no contracts)")
    except Exception as e:
    ...
    try:
        if contracts:
            ...
            print("     Skipped notification logs (no contracts)")
    except Exception as e:
    ...
    try:
        if contracts:
            ...
            print("   Skipped workflow events (no contracts)")
    except Exception as e:
    ...
    # 13. Designations
    try:
        await create_designations()
    except Exception as e:
        print(f"Designations failed: {e}")
        errors.append(f"Designations: {e}")

    # 14. Attendance records
    try:
        if employees:
            await create_attendance_records(employees, admins)
        else:
            print("   Skipped attendance (no employees)")
    except Exception as e:
        print(f"Attendance failed: {e}")
        errors.append(f"Attendance: {e}")

    # 15. Overtime & deductions
    try:
        if employees:
            await create_overtime_and_deductions(employees)
        else:
            print("   Skipped overtime/deductions (no employees)")
    except Exception as e:
        print(f"Overtime/deductions failed: {e}")
        errors.append(f"Overtime/deductions: {e}")

    # 16. Invoices (finance dashboard data)
    try:
        if contracts and admins:
            await create_invoices(contracts, admins)
        else:
            print("   Skipped invoices (no contracts)")
    except Exception as e:
        print(f"Invoices failed: {e}")
        errors.append(f"Invoices: {e}")

    # 17. Maintenance logs (real vehicle alerts)
    try:
        if vehicles:
            await create_maintenance_logs(vehicles)
        else:
            print("   Skipped maintenance logs (no vehicles)")
    except Exception as e:
        print(f"Maintenance logs failed: {e}")
