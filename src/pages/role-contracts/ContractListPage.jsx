...
const ContractListPage = () => {
  ...
  // Read initial filter from URL — e.g. /contracts?status=PENDING_APPROVAL
  const [filters, setFilters] = useState(() => {
    const urlStatus = searchParams.get('status') || '';
    return { ...DEFAULT_FILTERS, status: urlStatus };
  });
  const [pagination, setPagination] = useState({ current: 1, pageSize: 20, total: 0 });
  const [viewMode, setViewMode] = useState('table'); // 'table' | 'grid'

  // Re-sync filter when user navigates via sidebar (URL changes without remount)
        ...
        <Row justify="space-between" align="middle">
          <Col>
            <div className="page-title">
              {filters.status === 'PENDING_APPROVAL' ? 'Pending Approvals' : 'Contracts'}
            </div>
            <div className="page-subtitle">
              {filters.status === 'PENDING_APPROVAL'
                ? 'Contracts awaiting approval'
                : 'Manage modular contract workflow system'}
            </div>
          </Col>
          <Col>
            <Space>
              ...
              <Button
                ...
                loading={loading}
              />
              {canManage && filters.status !== 'PENDING_APPROVAL' && (
                <Button
                  type="primary"
                  icon={<PlusOutlined />}
